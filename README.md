...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/GoogleCloud-%234285F4.svg?style=for-the-badge&logo=google-cloud&logoColor=white" alt="Google Document AI" />
</div>
...
```mermaid
graph TD
    Client[📱 React Native Mobile App] --> API[🟢 Node.js API Server]
    
    API --> DB[(🐘 Neon PostgreSQL <br> + pgvector)]
    API --> Storage[☁️ Cloudflare R2]
    API --> AI[🧠 AI Services]
    
    AI -.-> OpenAI[OpenAI APIs]
    AI -.-> DocAI[Google Document AI]
    
    Client -.-> Auth[🔐 Supabase Auth]
    Auth -.-> API
    
    classDef client fill:#e1f5fe,stroke:#03a9f4,stroke-width:2px,color:#000;
    classDef api fill:#e8f5e9,stroke:#4caf50,stroke-width:2px,color:#000;
    classDef db fill:#fff3e0,stroke:#ff9800,stroke-width:2px,color:#000;
    classDef storage fill:#ede7f6,stroke:#673ab7,stroke-width:2px,color:#000;
    classDef ai fill:#fce4ec,stroke:#e91e63,stroke-width:2px,color:#000;
    classDef auth fill:#e0f7fa,stroke:#00bcd4,stroke-width:2px,color:#000;

    class Client client;
    class API api;
    class DB db;
    class Storage storage;
    class AI,OpenAI,DocAI ai;
    class Auth auth;
```

  ...
  return (
    <div ref={containerRef} className="flex-1 flex flex-col min-w-0 bg-[#0d0f14] relative">
      ...
      <div className={`flex-1 flex flex-col min-h-0 ${!isViewableUrl ? 'h-full' : ''}`}>
        <div className="flex items-center justify-between px-3 sm:px-6 py-1.5 sm:py-2 border-b border-white/5 bg-[#171920]">
          <div className="flex gap-2 sm:gap-4 overflow-x-auto no-scrollbar flex-1 min-w-0">
            {tabs.map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`py-1.5 sm:py-2 text-xs sm:text-sm font-semibold transition-colors border-b-2 whitespace-nowrap ${activeTab === tab.id ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-2 sm:ml-4">
            {activeTab === 'notes' && !isEditing ? (
              <button onClick={() => setIsEditing(true)} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-[11px] sm:text-xs font-medium transition-colors">
                <Edit3 className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> <span className="hidden sm:inline">Edit Mode</span><span className="sm:hidden">Edit</span>
              </button>
            ) : activeTab === 'notes' && isEditing ? (
              isCommunityNote ? (
                <>
                  <button onClick={handleSaveToMyNotes} disabled={isSavingToMyNotes || isSaving} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 text-[11px] sm:text-xs font-medium transition-colors">
                    {isSavingToMyNotes ? <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" /> : <Save className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} <span className="hidden sm:inline">Save to My Notes</span><span className="sm:hidden">Save Copy</span>
                  </button>
                  {canUpdateCommunityNote && (
                    <button onClick={handleSave} disabled={isSaving || isSavingToMyNotes} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 text-[11px] sm:text-xs font-medium transition-colors">
                      {isSaving ? <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" /> : <Save className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} <span className="hidden sm:inline">Update in Community</span><span className="sm:hidden">Update</span>
                    </button>
                  )}
                </>
              ) : (
                <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 text-[11px] sm:text-xs font-medium transition-colors">
                  {isSaving ? <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-spin" /> : <Save className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} Save
                </button>
              )
            ) : null}

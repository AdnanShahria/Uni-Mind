...
export const MessageInput = ({
  ...
  // If ANY tools or files are active, or user is typing, push the layout into "active" mode
  // Active mode means tools move to the top row, making room for badges/files, and input takes full width
  const isActiveTyping = input.length > 0 || attachedFiles.length > 0 || activeTools.length > 0;
          ...
          {showMenu && (
            ...
            >
              ...
              {setActiveTools && (
                <>
                  <button
                    onClick={() => toggleTool('fast_research')}
                    className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/[0.06] transition-colors text-left group w-full"
                  >
                    <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${activeTools.includes('fast_research') ? 'bg-purple-500/20 text-purple-400' : 'bg-white/5 text-slate-400 group-hover:text-purple-400'}`}>
                      <Sparkles className="w-3.5 h-3.5" />
                    </div>
                    <div className="flex-1">
                      <p className={`text-xs font-medium font-poppins ${activeTools.includes('fast_research') ? 'text-purple-400' : 'text-slate-300 group-hover:text-white'}`}>Fast Research</p>
                      <p className="text-[9px] text-slate-500 mt-0.5">Search your local notes</p>
                    </div>
                  </button>

                  <button
                    onClick={() => toggleTool('expanded_thinking')}
                    className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/[0.06] transition-colors text-left group w-full"
                  >
                    <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${activeTools.includes('expanded_thinking') ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-slate-400 group-hover:text-emerald-400'}`}>
                      <Brain className="w-3.5 h-3.5" />
                    </div>
                    <div className="flex-1">
                      <p className={`text-xs font-medium font-poppins ${activeTools.includes('expanded_thinking') ? 'text-emerald-400' : 'text-slate-300 group-hover:text-white transition-colors'}`}>Expanded Thinking</p>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/[0.06] transition-colors text-left group w-full"
                  >
                    <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${activeTools.includes('web_search') ? 'bg-cyan-500/20 text-cyan-400' : 'bg-white/5 text-slate-400 group-hover:text-cyan-400'}`}>
                      <Globe className="w-3.5 h-3.5" />
                    </div>
                    <div className="flex-1">
                      <p className={`text-xs font-medium font-poppins ${activeTools.includes('web_search') ? 'text-cyan-400' : 'text-slate-300 group-hover:text-white transition-colors'}`}>Web Search</p>
  ...
  return (
    <div className="shrink-0 px-2 pb-2 md:px-6 md:pb-6 relative z-20">
      <motion.div layout className="max-w-4xl mx-auto flex flex-col gap-1.5 md:gap-2">
        ...
        {/* Top Row: Holds the tools (when typing or active) and file attachments */}
        <motion.div layout className="flex items-center gap-2 min-h-[1px]">
          {isActiveTyping && <ToolsButtons isTop={true} />}
          
          <motion.div layout className="flex items-center gap-1.5 z-10 overflow-x-auto scrollbar-none flex-1 min-w-0 pb-0.5">
            <AnimatePresence>
              {activeTools.includes('fast_research') && (
                <motion.div 
                  key="tool-fast-research"
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  className="flex items-center gap-1.5 bg-purple-500/20 border border-purple-500/30 px-2.5 py-1 rounded-lg shadow-sm w-fit shrink-0"
                >
                  <Sparkles className="w-3.5 h-3.5 text-purple-400 shrink-0" />
                  <span className="text-[11px] text-purple-200 font-medium whitespace-nowrap">Fast Research</span>
                  <button
                    onClick={() => toggleTool('fast_research')}
                    className="text-purple-400 hover:text-purple-200 transition-colors ml-0.5 shrink-0 p-0.5 rounded-md hover:bg-purple-500/20"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
              )}
              {activeTools.includes('expanded_thinking') && (
                <motion.div 
                  key="tool-expanded-thinking"
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  className="flex items-center gap-1.5 bg-emerald-500/20 border border-emerald-500/30 px-2.5 py-1 rounded-lg shadow-sm w-fit shrink-0"
                >
                  <Brain className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                  <span className="text-[11px] text-emerald-200 font-medium whitespace-nowrap">Expanded Thinking</span>
                  <button
                    onClick={() => toggleTool('expanded_thinking')}
                    className="text-emerald-400 hover:text-emerald-200 transition-colors ml-0.5 shrink-0 p-0.5 rounded-md hover:bg-emerald-500/20"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
              ...
              {activeTools.includes('web_search') && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  className="flex items-center gap-1.5 bg-cyan-500/20 border border-cyan-500/30 px-2.5 py-1 rounded-lg shadow-sm w-fit shrink-0"
                >
                  <Globe className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                  <span className="text-[11px] text-cyan-200 font-medium whitespace-nowrap">Web Search</span>
                  <button
                    onClick={() => toggleTool('web_search')}
                    className="text-cyan-400 hover:text-cyan-200 transition-colors ml-0.5 shrink-0 p-0.5 rounded-md hover:bg-cyan-500/20"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
              ...
              {attachedFiles.map((file, idx) => (
                <motion.div 
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  className="flex items-center gap-1.5 bg-slate-800/90 backdrop-blur-xl border border-white/[0.08] px-2.5 py-1 rounded-lg shadow-sm w-fit max-w-[140px] shrink-0"
                >
                  <FileText className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                  <span className="text-[11px] text-slate-200 font-medium truncate min-w-0">{file.name}</span>
                  <button
                    onClick={() => onFileRemove && onFileRemove(idx)}
                    className="text-slate-400 hover:text-rose-400 transition-colors ml-0.5 shrink-0 p-0.5 rounded-md hover:bg-white/5"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </motion.div>
